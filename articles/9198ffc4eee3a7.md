...
## はじめに
メール配信サービスの移行をしたので、意識した事、感じた事など残しておこうと思います。
メール周りは低レイヤの通信プロトコルこそ実態はほぼほぼ統一されているといえど、インボックスプロパイダにより独自のレピュテーション基準があったり、配信サービスとしてもバウンス判定がバラバラであったりするためまだまだ仕組みを理解しきろうとするのには難しさを感じます。
...
バウンス率に関しては3%未満を理想とし、10%到達してしまうと危険水域という認識で進めており、現行システムでの配信エラー率が3%未満であったため、載せ替え時においてのクリーニングは実施しないという判断。
...
## 移行していく上で困ったこと
- SendGrid本体との契約で導入を検討していたが、アカウント開設後数十秒でブロックされてしまう[事象](https://www.reddit.com/r/SendGrid/comments/1arxlpf/sendgrid_dumped_me_and_i_never_sent_an_email/?rdt=36534)が発生。審査プロセスが非公開ということで解決に難航しそうだったため、代理店契約とした。
...
- ワンクリック解除のベストプラクティスに則り、メールヘッダにList-Unsubscribe-Post及びList-Unsubscribeの設定をするも、メーリングリストの登録解除ボタンが表示されない。Gmailコミュニティにある程度実績が積まれないと表示されないという[回答](https://support.google.com/mail/thread/49653586/list-unsubscribe-header-not-providing-the-option-to-unsubscribe?hl=en)もあったのでこちらも経過観察とし、本文内にリンクを設けておく対応とした。
    - ちなみにこちらはSendGrid側でヘッダが自動付与されるUnsubscribe Groupsの機能を利用しても同事象となった。
...
移行計画の精度が低かったと感じる。
リリース1週前になってIPウォームアップと送信先リストのクリーニングの観点から計画の変更が必要かどうか検討する流れとなってしまったので、他サービスの移行に関しても特に事故れない領域に関しては情報収集を予め厚めにしていく。